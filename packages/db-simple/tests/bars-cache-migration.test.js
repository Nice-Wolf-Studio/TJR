...
test('bars_cache migration: data insertion and retrieval', async () => {
  ...
  try {
    ...
    assert.strictEqual(aaplBars[0].timestamp, 1672531200000, 'First bar should have correct timestamp')
    assert.strictEqual(aaplBars[1].timestamp, 1672531200000, 'Second bar should have same timestamp (different provider)')
    assert.strictEqual(aaplBars[2].timestamp, 1672531500000, 'Third bar should have later timestamp')
...
test('bars_cache migration: metadata fields', async () => {
  ...
  try {
    ...
    const customRow = await db.query(
      'SELECT * FROM bars_cache WHERE symbol = ? AND provider = ?',
      ['TSLA', 'polygon']
    )
    assert.strictEqual(customRow[0].revision, 2, 'revision should be set to custom value')
    assert.strictEqual(customRow[0].providerPriority, 50, 'providerPriority should be set to custom value')
  } finally {
    await db.close()
  }
})