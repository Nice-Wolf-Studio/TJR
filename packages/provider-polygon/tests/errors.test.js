...
describe('RateLimitError - Construction', () => {
  it('should create RateLimitError with full data', () => {
    ...
    assert.ok(error.message.includes('rate limit'), 'Message should mention rate limit');
    assert.equal(error.data.retryAfter, 60, 'Should have retryAfter in data');
    assert.equal(error.data.limitType, 'requests_per_minute', 'Should have limitType in data');
    assert.equal(
...
describe('ParseError - Construction', () => {
  it('should create ParseError with full data', () => {
    ...
    assert.ok(error.message.includes('Missing required field'), 'Message should describe error');
    assert.equal(
      error.data.responseBody,
      '{"status": "OK", "ticker": "ES"}',
      'Should have responseBody in data'
...
describe('isRateLimitError - Type Guard', () => {
  ...
  it('should return false for ParseError', () => {
    ...
    assert.equal(
      ...
      false,
      'Should not identify ParseError as RateLimitError'
    );
  });
...
describe('Error Use Cases', () => {
  ...
  it('should support error handling pattern', () => {
    ...
    assert.equal(handleError(apiError), 'API error: 401 - Unauthorized', 'Should handle ApiError');
