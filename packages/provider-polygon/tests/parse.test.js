...
describe('parseAggregatesResponse - Happy Path', () => {
  it('should parse 1-minute intraday bars from fixture', () => {
    ...
    assert.equal(bars[0].open, 4500.0, 'First bar open should match');
    assert.equal(bars[0].high, 4502.0, 'First bar high should match');
    assert.equal(bars[0].low, 4499.5, 'First bar low should match');
    assert.equal(bars[0].close, 4501.5, 'First bar close should match');
  ...
  it('should parse daily bars from fixture', () => {
    ...
    assert.equal(bars[0].open, 4495.0, 'First bar open should match');
    assert.equal(bars[0].high, 4510.0, 'First bar high should match');
    assert.equal(bars[0].low, 4490.0, 'First bar low should match');
    assert.equal(bars[0].close, 4505.0, 'First bar close should match');
...
describe('parseAggregatesResponse - Timezone Handling', () => {
  it('should preserve Unix millisecond timestamps without timezone conversion', () => {
    ...
    assert.equal(bars[0].timestamp, 1727787000000, 'Timestamp should be preserved as-is from API');
