...
describe("createPolygonProvider - Configuration", () => {
  it("should create provider with valid config", () => {
    ...
    assert.equal(typeof provider.capabilities, "function", "Provider should have capabilities method");
  });
...
describe("provider.capabilities", () => {
  it("should return correct capabilities", () => {
    ...
    assert.ok(
      caps.supportsTimeframes.includes("1m"),
      "Should support 1m timeframe"
    );
    assert.ok(
      caps.supportsTimeframes.includes("5m"),
      "Should support 5m timeframe"
    );
    assert.ok(
      caps.supportsTimeframes.includes("10m"),
      "Should support 10m timeframe (aggregated)"
    );
    assert.ok(
      caps.supportsTimeframes.includes("4h"),
      "Should support 4h timeframe (aggregated)"
    );
    assert.ok(
      caps.supportsTimeframes.includes("1D"),
      "Should support 1D timeframe"
    );
    ...
    assert.equal(caps.rateLimits.requestsPerMinute, 5, "Rate limit should be 5 req/min (free tier)");
...
describe("provider.getBars - Native Timeframes (5m)", () => {
  it("should fetch 5m bars with mocked client", async () => {
    ...
    assert.equal(bars[0].open, 4500.00, "First bar open should match fixture");
...
describe("provider.getBars - Aggregated Timeframes (10m)", () => {
  it("should fetch and aggregate 5m → 10m", async () => {
    ...
    assert.equal(bars[0].open, 4500.00, "First 10m bar open should match first 5m bar");
    assert.equal(bars[0].close, 4502.00, "First 10m bar close should match second 5m bar");
    assert.equal(bars[0].volume, 153000, "First 10m bar volume should sum first two 5m bars (75000 + 78000)");
  });
});
...
describe("provider.getBars - Date Conversion", () => {
  it("should convert Date objects to Unix milliseconds", async () => {
    ...
    mock.method(PolygonClient.prototype, "getAggregates", async (_symbol, _mult, _span, from, to) => {
      capturedFrom = from;
      capturedTo = to;
      return fixture;
    });
