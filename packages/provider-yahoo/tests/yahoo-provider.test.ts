...
describe('YahooProvider', () => {
  ...
  describe('getBars', () => {
    it('should fetch 1m bars from fixture', async () => {
      ...
      expect(bars[0].open).toBe(4750.00);
    ...
    it('should aggregate 1m to 10m bars', async () => {
      ...
      expect(firstBar.open).toBe(4750.00); // First bar's open
    ...
    it('should aggregate 1h to 4h bars', async () => {
      ...
      const firstBar = bars[0];
      expect(firstBar.open).toBe(4700.00); // First hour's open
    ...
    it('should filter bars by date range', async () => {
      ...
      bars.forEach(bar => {
        ...
        expect(barDate.getTime()).toBeGreaterThanOrEqual(new Date('2024-01-15T14:30:00.000Z').getTime());
        expect(barDate.getTime()).toBeLessThanOrEqual(new Date('2024-01-15T14:35:00.000Z').getTime());
      });
    });
  ...
  describe('parseYahooBar', () => {
    it('should parse valid Yahoo bar', () => {
      ...
      expect(bar.open).toBe(4750.00);
      expect(bar.high).toBe(4752.25);
      expect(bar.low).toBe(4749.50);
