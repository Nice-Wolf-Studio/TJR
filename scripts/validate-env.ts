 ...
 *
 * Usage:
 *   node scripts/validate-env.ts [--env=production|staging|development]
 ...
 */
function validateSecret(validator: SecretValidator): ValidationResult {
 ...
 */
function validateDiscordEnv(env: 'DEV' | 'STAGE' | 'PROD'): ValidationResult {
  const result: ValidationResult = { valid: true, errors: [], warnings: [] };
...
function validate(): boolean {
  ...
  const discordEnvs: Array<'DEV' | 'STAGE' | 'PROD'> = ['DEV', 'STAGE', 'PROD'];
